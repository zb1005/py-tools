...
def generate_report_1(merged_data):
    ...
    report = merged_data.groupby('审批人所在体系').agg(
        **{
            'A-审批总次数': ('流程名称', 'count'),
            'B-单个节点审批时长≤1天的节点数': ('节点审批时效情况（≤1；＞1）', lambda x: (x == '<=1').count()),
            'D-单个节点审批时长＞1天的节点数': ('节点审批时效情况（≤1；＞1）', lambda x: (x == '>1').count()),
            'E-其中：单个节点审批时长>3天的节点数': ('节点审批时效是否大于3天', lambda x: (x == 'Y').count()),
...
def process_approval_data():
    # 读取四个Excel文件
    base_df = pd.read_excel(r"C:\Users\zhangbon\Desktop\Q1审批时效数据分析打样-0513.xlsx",sheet_name="主表", engine='openpyxl')
    staff_df = pd.read_excel(r"C:\Users\zhangbon\Desktop\Q1审批时效数据分析打样-0513.xlsx",sheet_name="附1 最新在职人员及所属组织清单", engine='openpyxl')
    holiday_df = pd.read_excel(r"C:\Users\zhangbon\Desktop\Q1审批时效数据分析打样-0513.xlsx",sheet_name="附2 方太春节假期", engine='openpyxl')
    special_node_df = pd.read_excel(r"C:\Users\zhangbon\Desktop\Q1审批时效数据分析打样-0513.xlsx",sheet_name="附3特殊节点合理时长", engine='openpyxl')
