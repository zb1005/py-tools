...
def compare_datasets(source_df, target_df, key_columns):
    ...
    # 处理共同主键的数据差异
    for key in tqdm(common_keys, desc="比对共同记录"):
        ...
        for col in common_columns:
            ...
            if src_empty and tgt_empty:
                ...
                diff_details[col] = {
                    '源值': src_val,
                    '目标值': tgt_val,
                    '归一化源值': normalized_src,
                    '归一化目标值': normalized_tgt
                }
...
def batch_compare(source_path, target_paths, key_columns, output_dir):
    ...
    try:
        ...
        # 遍历目标文件
        for target_path in tqdm(target_paths, desc="处理目标文件"):
