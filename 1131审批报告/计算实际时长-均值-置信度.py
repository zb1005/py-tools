...
holiday_df = pd.read_excel(r"C:\Users\zhangbon\Desktop\临时活\1131统计\计算节点合理时长\2024-方太非工作日清单0528.xlsx", engine='openpyxl')
holidays = [datetime.strptime(str(date).strip(), '%Y-%m-%d %H:%M:%S').date() for date in holiday_df['日期']]

df = pd.read_excel(r"C:\Users\zhangbon\Desktop\临时活\1131统计\计算节点合理时长\Q1-PBC审批流程.xlsx", engine='openpyxl')
...
df.to_excel(r"C:\Users\zhangbon\Desktop\临时活\1131统计\计算节点合理时长\Q1-PBC审批流程-计算工作时长.xlsx", index=False)
print(df.columns)
grouper = analyze_approval_duration(df, '流程名称', '审批节点名称')
grouper.to_excel(r"C:\Users\zhangbon\Desktop\临时活\1131统计\计算节点合理时长\Q1-PBC审批流程-分析结果稿.xlsx", index=False)
print("分析完成")
